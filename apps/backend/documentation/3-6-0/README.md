...
npm run migrate:3.6.0

# Or run directly
node documentation/3-6-0/3-6-0-migration.js
```
...
```bash
# With local MongoDB
MONGO_DB_HOST=localhost MONGO_DB_PORT=27017 node documentation/3-6-0/3-6-0-migration.js

# With remote MongoDB
MONGO_DB_HOST=mongodb-cluster.com MONGO_DB_PORT=27017 node documentation/3-6-0/3-6-0-migration.js
```
...
- **Main Orchestrator**: `3-6-0-migration.js` is a minimal orchestrator that provides guidance and migration status
- **Standalone Execution**: Each migration file can be run independently

## Orchestrator Approach

The `3-6-0-migration.js` file serves as a Node.js-based orchestrator that:
...
To add a new migration to the 3.6.0 suite:

1. Create a new `.js` file in this directory with the migration logic
2. Export the main migration function from the file
3. Add the migration to the `availableMigrations` array in `3-6-0-migration.js`
...
Then create an execute function:

```javascript
async function executeYourMigration(db) {
    console.log('🔄 Executing your migration...');
    
    try {
        const migration = require('./your-migration-file');
        const result = await migration.yourMigrationFunction(db);

        if (result.success) {
            console.log('✅ Your migration completed successfully');
        } else {
            console.log('❌ Your migration failed');
        }
