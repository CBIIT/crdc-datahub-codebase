...
class GenerateDCF:
    ...
    def _build_rows(self):
        """Returns (rows, columns) on success, or (None, None) after logging the reason."""
    ...
    def generate_dcf_preview(self):
        ...
        try:
            ...
            df.to_csv(buf, sep='\t', index=False)
            buf.seek(0)
            root_path, bucket_name = self.submission[ROOT_PATH], self.submission[BATCH_BUCKET]
            self.s3_service.upload_file_to_s3(buf, bucket_name, f"{root_path}/dcf_manifest.tsv")
            self.log.info(f"{self.submission[ID]}: DCF manifest preview uploaded.")
            return True
        except Exception as e:
            self.log.exception(e)
            self.log.exception(f'{self.submission[ID]}: Failed to generate DCF manifest preview: {get_exception_msg()}.')
            return False
        finally:
            if buf:
                del buf
