...
class metadatavalidationService:
  def createService(self, config):
    ...
    taskDefinition = ecs.FargateTaskDefinition(self,
        ...
        family=f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadatavalidation",
    ...
    secrets={
            "NEW_RELIC_LICENSE_KEY":ecs.Secret.from_secrets_manager(secretsmanager.Secret.from_secret_name_v2(self, "metadata_newrelic", secret_name='monitoring/newrelic'), 'api_key'),
    ...
    if(config['main']['create_bucket'] == "true"):
        bucket = s3.Bucket.from_bucket_name(self, f"{self.namingPrefix}-submission-metadata-ref", f"{self.namingPrefix}-submission")
    else:
        existing_bucket_name = config["secrets"].get("submission_bucket")
        bucket = s3.Bucket.from_bucket_name(self, f"{self.namingPrefix}-submission-metadata-existing", existing_bucket_name)
    ...
    for bucket_name in bucket_names:
        bucket_arns_meta.append(f"arn:aws:s3:::{bucket_name}")
        bucket_arns_meta.append(f"arn:aws:s3:::{bucket_name}/*")
    ...
    scale_out_policy = appscaling.CfnScalingPolicy(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-out-policy",                                                   
        policy_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-out-policy",
    ...
    scale_out_alarm = cloudwatch.CfnAlarm(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-out-alarm",                                   
        alarm_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-out-alarm",
        namespace="AWS/SQS",
        metric_name="ApproximateNumberOfMessagesVisible",
    ...
    scale_in_policy = appscaling.CfnScalingPolicy(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-in-policy",                                               
        policy_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-in-policy",
        policy_type="StepScaling",
        resource_id=ecs_resource_id,
        scalable_dimension="ecs:service:DesiredCount",
        service_namespace="ecs",
    ...
    scale_in_alarm = cloudwatch.CfnAlarm(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-in-alarm",                                     
        alarm_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-scale-in-alarm",
        namespace="AWS/SQS",
        metric_name="ApproximateNumberOfMessagesVisible",
        dimensions=[cloudwatch.CfnAlarm.DimensionProperty(name="QueueName", value=queue.queue_name)],
        statistic="Minimum",
    ...
    if env.lower() != 'prod':
        scalable_target.scale_on_schedule(
            f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-start",
        ...
        scalable_target.scale_on_schedule(
            f"{config['main']['resource_prefix']}-{config['main']['tier']}-metadata-stop",
            schedule=appscaling.Schedule.cron(
                minute="0",
                hour="23",
                week_day="MON-FRI"
