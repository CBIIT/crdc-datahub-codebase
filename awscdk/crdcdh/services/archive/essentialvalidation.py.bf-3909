...
class essentialvalidationService:
  def createService(self, config):
    ...
    if(config['main']['create_bucket'] == "true"):
        bucket = s3.Bucket.from_bucket_name(self, f"{self.namingPrefix}-submission-essential-ref", f"{self.namingPrefix}-submission")
    else:
        existing_bucket_name = config["secrets"].get("submission_bucket")
        bucket = s3.Bucket.from_bucket_name(self, f"{self.namingPrefix}-submission-essential-existing", existing_bucket_name)
    ...
    bucket_arns_essential = []
    for bucket_name in bucket_names:
        bucket_arns_essential.append(f"arn:aws:s3:::{bucket_name}")
        bucket_arns_essential.append(f"arn:aws:s3:::{bucket_name}/*")
    ...
    data_sync_other_buckets = iam.PolicyStatement(
        ...
        resources=bucket_arns_essential
    )
    ...
    scale_out_policy = appscaling.CfnScalingPolicy(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-out-policy",
        policy_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-out-policy",                                              
    ...
    scale_out_alarm = cloudwatch.CfnAlarm(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-out-alarm",
    ...
    scale_in_policy = appscaling.CfnScalingPolicy(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-in-policy",
        policy_name=f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-in-policy",
        policy_type="StepScaling",
        resource_id=ecs_resource_id,
        scalable_dimension="ecs:service:DesiredCount",
        service_namespace="ecs",
    ...
    scale_in_alarm = cloudwatch.CfnAlarm(self,
        f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-scale-in-alarm",
    ...
    if env.lower() != 'prod':
        scalable_target.scale_on_schedule(
            f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-start",
        ...
        scalable_target.scale_on_schedule(
            f"{config['main']['resource_prefix']}-{config['main']['tier']}-essential-stop",
            schedule=appscaling.Schedule.cron(
                minute="0",
                hour="23",
                week_day="MON-FRI"
